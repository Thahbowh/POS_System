...
───────────────────────────────────────── */
const catConfig = {
  beer:        { emoji: '<i data-lucide="beer"></i>', label: "Beer",       cls: "cat-beer"       },
  cider:       { emoji: '<i data-lucide="apple"></i>', label: "Cider",      cls: "cat-cider"      },
  food:        { emoji: '<i data-lucide="utensils"></i>', label: "Food",        cls: "cat-food"       },
  cigarettes: { emoji: '<i data-lucide="cigarette"></i>', label: "Cigarettes", cls: "cat-cigarettes"},
};

function getCat(key) {
  return catConfig[key] || { emoji: '<i data-lucide="package"></i>', label: key || "Other", cls: "cat-default" };
}
...
function renderProductGrid(products) {
  ...
  grid.innerHTML = products.map(p => {
    ...
    const stockTxt = p.stock <= 0
      ? '<i data-lucide="x"></i> Out of stock'
      : p.stock <= threshold
      ? `<i data-lucide="alert-triangle"></i> Only ${p.stock} left`
      : `<i data-lucide="check"></i> ${p.stock} in stock`;
      ...
      <div class="prod-card">
        ...
        <div class="prod-card-body">
          ...
          <div class="prod-card-actions">
            <button class="prod-edit-btn" onclick="openModal(${JSON.stringify(p).split('"').join('&quot;')})"><i data-lucide="square-pen"></i> Edit</button>
            <button class="prod-del-btn"  onclick="deleteProduct(${p.id})"><i data-lucide="trash-2"></i></button>
          </div>
        </div>
      </div>
    `;
...
function renderProductList(products) {
  ...
  tbody.innerHTML = products.map(p => {
    ...
    const stockTxt = p.stock <= 0 ? "Out of stock"
      : p.stock <= thresh ? `<i data-lucide="alert-triangle"></i> ${p.stock} left`
      : `<i data-lucide="check"></i> ${p.stock}`;
      ...
      <tr>
        ...
        <td>
          <div class="action-group">
            <button class="prod-edit-btn" style="padding:5px 10px;" onclick="openModal(${JSON.stringify(p).split('"').join('&quot;')})"><i data-lucide="square-pen"></i> Edit</button>
            <button class="prod-del-btn"  style="padding:5px 10px;" onclick="deleteProduct(${p.id})"><i data-lucide="trash-2"></i></button>
          </div>
        </td>
      </tr>
    `;
...
function switchTo(id, childEl) {
  ...
  // Open the parent group of this child
  if (childEl) {
    const group = childEl.closest(".nav-children");
  ...
  // Update topbar title
  const titles = {
    dashboard: "Control Panel", orders: "Orders",
    scanner: "New Sale", products: "Products",
    expenses: "Expenses & Profit", users: "Users",
    analytics: "Analytics", settings: "Settings"
  };
  const titleEl = document.querySelector(".topbar-title");
  if (titleEl) titleEl.textContent = titles[id] || id;

  // Keep existing init hooks working
  if (id === "orders")   initOrdersTab?.();
  if (id === "products") loadProducts?.();
  if (id === "users")    loadUsers?.();
  if (id === "analytics") renderGraph?.(currentRange);
...
function showSection(id, el) { switchTo(id, el); }  
...
const ordStatusCfg = {
  pending:   { label: '<i data-lucide="hourglass"></i> Pending',   cls: "pending"   },
  completed: { label: '<i data-lucide="check"></i> Completed',  cls: "completed" },
  cancelled: { label: '<i data-lucide="x"></i> Cancelled',  cls: "cancelled" },
};
 
const payLabels = {
  cash:   { label: '<i data-lucide="banknote"></i> Cash',   cls: "pay-cash"   },
  card:   { label: '<i data-lucide="credit-card"></i> Card',   cls: "pay-card"   },
  online: { label: '<i data-lucide="globe"></i> Online', cls: "pay-online" },
};
  ...
  tbody.innerHTML = orders.map(order => {
    ...
    return `
      <tr class="${rowCls}">
        ...
        <td>
          <div class="action-group">
            <button class="act-btn view"     onclick="openOrderDetail(${idx})"><i data-lucide="eye"></i> View</button>
            <button class="act-btn complete" onclick="changeOrderStatus(${idx},'completed')" ${isDone ? 'disabled' : ''}><i data-lucide="check-circle"></i></button>
            <button class="act-btn cancel"   onclick="changeOrderStatus(${idx},'cancelled')" ${isDone ? 'disabled' : ''}><i data-lucide="x-circle"></i></button>
          </div>
        </td>
      </tr>
    `;
...
function openOrderDetail(idx) {
  ...
  if (o.status === "pending") {
    actionsEl.innerHTML = `
      ...
      <button class="act-btn cancel"   onclick="changeOrderStatus(${idx},'cancelled'); closeOrderModal()"><i data-lucide="x"></i> Cancel</button>
      <button class="act-btn complete" onclick="changeOrderStatus(${idx},'completed'); closeOrderModal()"><i data-lucide="check-circle"></i> Mark Completed</button>
    `;
  } else {
...
function exportOrdersCSV() {
  ...
  showToast('Orders exported <i data-lucide="check"></i>');
}
...
function renderGraph(range) {
  ...
  if (currentView === "peak") {
    ...
    const changeEl = document.getElementById("graph-change");
    if (changeEl) {
      changeEl.innerHTML = peak >= 0 ? `<i data-lucide="flame"></i> Busiest: ${d.labels[peak]}` : "";
      if (typeof lucide !== 'undefined') lucide.createIcons();
...
const statusConfig = {
  completed:  { label: '<i data-lucide="check"></i> Completed',   cls: "completed"  },
  pending:    { label: '<i data-lucide="hourglass"></i> Pending',     cls: "pending"    },
  processing: { label: '<i data-lucide="settings"></i> Processing',  cls: "processing" },
  cancelled:  { label: '<i data-lucide="x"></i> Cancelled',   cls: "cancelled"  },
};
      ...
      <tr>
        ...
        <td>
          <div class="action-group">
            ...
            ${done ? 'disabled style="opacity:0.3;cursor:not-allowed;"' : ''}><i data-lucide="check"></i></button>
            <button class="act-btn cancel"   onclick="updateOrderStatus('${order.id}','cancelled')"
            ${done ? 'disabled style="opacity:0.3;cursor:not-allowed;"' : ''}><i data-lucide="x"></i></button>
...
function renderUsersTable(users) {
  ...
  tbody.innerHTML = users.map(u => {
    ...
    const roleIcon= u.role === "admin" ? '<i data-lucide="crown"></i>' : '<i data-lucide="user"></i>';
        ...
        <td>
          <div class="action-group">
            <button class="act-btn view"
              onclick="openUserModal(${u.id})"><i data-lucide="square-pen"></i> Edit</button>
...
async function saveUser() {
  ...
  try {
    ...
    showToast(id ? '<i data-lucide="check-circle"></i> User updated' : '<i data-lucide="check-circle"></i> User created');
    closeUserModal();
    loadUsers();
...
async function deleteUser(id, name) {
  try {
    ...
    showToast(`<i data-lucide="trash-2"></i> ${name} deleted`);
    loadUsers();
...
function buildAnalyticsData(range) {
  ...
  if (revenue === 0 && orders === 0) {
    alerts.push({ type:'info', icon:'<i data-lucide="bar-chart-3"></i>', text:'No orders in this period yet — try a wider range.' });
  } else {
    if (kpi.revenue.change.dir === 'up')
      alerts.push({ type:'success', icon:'<i data-lucide="trending-up"></i>', text:`Revenue is up vs the previous period — keep it up!` });
    if (kpi.revenue.change.dir === 'down')
      alerts.push({ type:'warn', icon:'<i data-lucide="trending-down"></i>', text:`Revenue is down vs the previous period. Check peak hours.` });
    if (peakHour.orders > 0)
      alerts.push({ type:'gold', icon:'<i data-lucide="flame"></i>', text:`Busiest time: ${peakHour.label} with ${peakHour.orders} orders — consider extra staff then.` });
    if (returning > 0)
      alerts.push({ type:'success', icon:'<i data-lucide="repeat"></i>', text:`${returning} returning customer${returning>1?'s':''} this period — loyalty is building.` });
  }
  ...
  if (outItems.length > 0)
    alerts.push({ type:'warn', icon:'<i data-lucide="alert-triangle"></i>', text:`${outItems.length} product${outItems.length>1?'s are':' is'} out of stock.` });
  else if (lowItems.length > 0)
    alerts.push({ type:'warn', icon:'<i data-lucide="package"></i>', text:`${lowItems.length} product${lowItems.length>1?'s are':' is'} running low on stock.` });
...
async function loadAnalytics(range = '7d') {
  ...
  if (hrsCtx) {
    ...
    if (insight) {
      insight.innerHTML =
        `<i data-lucide="flame"></i> Peak time is ${data.peakHour.label} with ${data.peakHour.orders} orders — schedule extra staff then.`;
      if (typeof lucide !== 'undefined') lucide.createIcons();
    }
  }
      ...
      <td>
        <div class="action-group">
          <button class="act-btn complete" ${o.status!=='pending'?'disabled':''}
            onclick="updateOrderStatus('${o.id}','completed')"><i data-lucide="check"></i></button>
          <button class="act-btn cancel" ${o.status!=='pending'?'disabled style="opacity:0.3;cursor:not-allowed;"':''}
            onclick="updateOrderStatus('${o.id}','cancelled')"><i data-lucide="x"></i></button>
...
const EXP_CATEGORIES = {
  rent:        { label: 'Rent',              icon: '<i data-lucide="house"></i>', color: '#60a5fa' },
  electricity: { label: 'Electricity',       icon: '<i data-lucide="zap"></i>', color: '#fbbf24' },
  salaries:    { label: 'Employee Salaries', icon: '<i data-lucide="users"></i>', color: '#a78bfa' },
  stock:       { label: 'Stock/Products',    icon: '<i data-lucide="package"></i>', color: '#34d399' },
  internet:    { label: 'Internet',          icon: '<i data-lucide="globe"></i>', color: '#22d3ee' },
  marketing:   { label: 'Marketing/Ads',     icon: '<i data-lucide="megaphone"></i>', color: '#f97316' },
  transport:   { label: 'Transport',         icon: '<i data-lucide="car"></i>', color: '#fb7185' },
  other:       { label: 'Other',             icon: '<i data-lucide="clipboard-list"></i>', color: '#94a3b8' },
};
...
function renderExpSummaryCards() {
  ...
  const profitIcon   = netProfit > 0 ? '<i data-lucide="trending-up"></i>' : netProfit < 0 ? '<i data-lucide="trending-down"></i>' : '<i data-lucide="scale"></i>';
  ...
  el.innerHTML = `
    <div class="exp-kpi-card">
      <div class="exp-kpi-icon" style="background:rgba(245,200,66,0.1);color:#f5c842;"><i data-lucide="banknote"></i></div>
    ...
    <div class="exp-kpi-card">
      <div class="exp-kpi-icon" style="background:rgba(239,68,68,0.1);color:#ef4444;"><i data-lucide="wallet"></i></div>
      <div class="exp-kpi-body">
    ...
    <div class="exp-kpi-card">
      <div class="exp-kpi-icon" style="background:rgba(251,146,60,0.1);color:#fb923c;"><i data-lucide="calendar"></i></div>
      <div class="exp-kpi-body">
    ...
    <div class="exp-kpi-card">
      <div class="exp-kpi-icon" style="background:rgba(167,139,250,0.1);color:#a78bfa;"><i data-lucide="calendar-days"></i></div>
      <div class="exp-kpi-body">
    ...
    <div class="exp-kpi-card">
      <div class="exp-kpi-icon" style="background:${mPColor}18;color:${mPColor};"><i data-lucide="calendar-range"></i></div>
      <div class="exp-kpi-body">
...
function renderExpAlerts() {
  ...
  if (totalExpenses === 0 && loadExpenses().length === 0) {
    alerts.push({ type: 'info', icon: '<i data-lucide="lightbulb"></i>', text: 'No expenses recorded yet. Add your first expense below to start tracking profitability.' });
  } else {
    if (margin < 10 && totalIncome > 0)
      alerts.push({ type: 'warn', icon: '<i data-lucide="alert-triangle"></i>', text: `Low profit margin warning: only ${margin.toFixed(1)}% margin. Review your expenses to improve profitability.` });
    if (netProfit < 0)
      alerts.push({ type: 'loss', icon: '<i data-lucide="trending-down"></i>', text: `Business is currently at a loss of R${Math.abs(netProfit).toLocaleString('en-ZA')}. Total expenses exceed income.` });
    if (prevMonthExp > 0 && monthExp > prevMonthExp * 1.2)
      alerts.push({ type: 'warn', icon: '<i data-lucide="bar-chart-3"></i>', text: `Monthly expenses increased ${Math.round(((monthExp - prevMonthExp) / prevMonthExp) * 100)}% vs last month. High spending detected.` });
    if (netProfit > 0 && margin >= 20)
      alerts.push({ type: 'success', icon: '<i data-lucide="target"></i>', text: `Healthy profit margin of ${margin.toFixed(1)}%. Business is performing well.` });
  }
...
function saveExpense() {
  ...
  if (expEditingId) {
    ...
    showToast('<i data-lucide="check-circle"></i> Expense updated', 'success');
  } else {
    expenses.unshift({ id: genExpenseId(), title, category, amount, date, notes, createdAt: new Date().toISOString() });
    showToast('<i data-lucide="check-circle"></i> Expense saved', 'success');
  }
      ...
      <tr class="exp-row" style="animation: rowIn 0.25s ease both;">
        ...
        <td>
          <div style="display:flex;gap:6px;">
            <button class="exp-btn-edit" onclick="openExpenseForm('${e.id}')"><i data-lucide="square-pen"></i> Edit</button>
            <button class="exp-btn-del"  onclick="deleteExpense('${e.id}')"><i data-lucide="trash-2"></i></button>
          </div>
        </td>
      </tr>
    `;
...
function exportExpensesCSV() {
  ...
  a.click();
  URL.revokeObjectURL(url);
  showToast('<i data-lucide="download"></i> Expenses exported to CSV', 'success');
}
...
function printFinancialReport() {
  ...
  w.document.write(`
    ...
    <style>
      ...
      h1   { font-size: 22px; border-bottom: 2px solid #111; padding-bottom: 10px; display: flex; align-items: center; gap: 8px; }
      ...
      td svg, th svg, h1 svg { width: 14px; height: 14px; vertical-align: middle; }
      @media print { body { margin: 0; } }
    </style></head><body>
    <h1><i data-lucide="bar-chart-3"></i> Financial Report</h1>
    ...
    <script>window.onload = () => { if (typeof lucide !== 'undefined') lucide.createIcons(); window.print(); };<\/script>
    </body></html>
  `);
  w.document.close();
}
